...
export class ProductsService {
  ...
  getAllAdvanceTables(): void {
    this.httpClient.get<any>(`${environment.apiUrl}/api/carousel`).subscribe(
      (res) => {
  ...
  addAdvanceTable(advanceTable: ProductsTable): void {
    if (advanceTable._id === "") {
      this.httpClient
        .post<any>(`${environment.apiUrl}/api/carousel`, advanceTable)
        .subscribe(
          (res) => {
            //console.log(res);
          },
      ...
      this.httpClient
        .put<any>(
          `${environment.apiUrl}/api/carousel/${advanceTable._id}`,
          advanceTable
        )
        .subscribe(
          (res) => {
  ...
  deleteAdvanceTable(id: string, name: string, status: number): void {
    this.httpClient
      .delete<any>(`${environment.apiUrl}/api/carousel/${id}`, {})
      .subscribe(
        (res) => {
          console.log(res);
        },
